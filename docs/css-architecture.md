...
styles/
  ...
  global.css      ← navigation, menu, form layout  [@layer components]
  app.css         ← view cards, form inputs, session screen  [@layer components]
  button.css      ← button base class + all variants  [@layer components]
  history.css     ← history view components        [@layer components]
```
...
graph TD
    ...
    subgraph components_layer ["@layer components"]
        global["global.css\nnav · mobile menu · form alignment"]
        app["app.css\nview cards · inputs · progress bar · session"]
        button["button.css\n.btn base + .btn--primary · --secondary · --menu"]
        history["history.css\nhistory entries · replay button · empty state"]
    end
...
.btn--primary   { /* solid dark teal, main CTA */ }
.btn--secondary { /* outlined, secondary action */ }
.btn--menu      { /* mobile nav drawer buttons */ }
```
